...
git status is a handy command to check the current repository status. It is recommonded it is used from time to time.
...
Type git status and you can see from console ouput that there are changes to be committed and no untracked files. (If you run the above command in sub folder, then only the files under subfolders will be tracked)
...
Now goto GitHub do the registration, create a repsitory on GitHub, and get the git URL of the repository.
...
You will be asked to enter your Github user name and credential. After the 'push' command suceeds, you can see the files appear on the Github repository.
